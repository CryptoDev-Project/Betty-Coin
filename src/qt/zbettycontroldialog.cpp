...
#include "ui_zbettycontroldialog.h"
...
std::list<std::string> ZBETTYControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBETTYControlDialog::listMints;

ZBETTYControlDialog::ZBETTYControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBETTYControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBETTYControlDialog::~ZBETTYControlDialog()
{
    delete ui;
}

void ZBETTYControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBETTYControlDialog::updateList()
{
...
void ZBETTYControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBETTYControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBETTY_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBETTYControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBETTYControlDialog::GetSelectedMints()
{
...
void ZBETTYControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
