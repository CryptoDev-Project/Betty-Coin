...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BETTY);
    ...
    if (!settings.contains("nAnonymizeBETTYAmount"))
        settings.setValue("nAnonymizeBETTYAmount", 1000);

    nAnonymizeBETTYAmount = settings.value("nAnonymizeBETTYAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBETTYAmount"))
        SoftSetArg("-anonymizebettyamount", settings.value("nAnonymizeBETTYAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in betty.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBETTYAmount:
            return QVariant(nAnonymizeBETTYAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBETTYAmount:
            nAnonymizeBETTYAmount = value.toInt();
            settings.setValue("nAnonymizeBETTYAmount", nAnonymizeBETTYAmount);
            emit anonymizeBETTYAmountChanged(nAnonymizeBETTYAmount);
            break;
        case CoinControlFeatures:
