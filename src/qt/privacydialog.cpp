                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBETTY ought to be enough for anybody." - Bill Gates, 2017
    ui->zBETTYpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBETTYSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBETTY->setEnabled(false);
        ui->pushButtonMintzBETTY->setToolTip(tr("zBETTY is currently disabled due to maintenance."));

        ui->pushButtonSpendzBETTY->setEnabled(false);
        ui->pushButtonSpendzBETTY->setToolTip(tr("zBETTY is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBETTYpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBETTY_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBETTY is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBETTY...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBETTY in ") +
...
void PrivacyDialog::on_pushButtonSpendzBETTY_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBETTY is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBETTY
        sendzBETTY();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBETTY
    sendzBETTY();
}

void PrivacyDialog::on_pushButtonZBETTYControl_clicked()
{
    ZBETTYControlDialog* zBETTYControl = new ZBETTYControlDialog(this);
    zBETTYControl->setModel(walletModel);
    zBETTYControl->exec();
}

void PrivacyDialog::setZBETTYControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBETTYSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBETTY()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid BETTY Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBETTYpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBETTY is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BETTY </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBETTY</b>";
    ...
    // use mints from zBETTY selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBETTYControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBETTYControlDialog::GetSelectedMints();
    }
    ...
    // Clear zbetty selector in case it was used
    ZBETTYControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBETTY Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " BETTY, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBETTY Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBETTY </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBETTY "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBETTY "));
    ui->labelzBETTYAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    // Display AutoMint status

    updateAutomintStatus();

    // Display global supply
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zBETTY </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zBETTY </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzBETTYSyncStatus->setVisible(fShow);
}
