...
#if defined(NDEBUG)
#error "BETTY cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge2 = 6 * 60 * 60; // 6 * 60 * 60
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zBETTY spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zBETTY spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zBETTY spend with serial %s from tx %s is not in valid range\n",
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from BETTY
...
{
    RenameThread("betty-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZBETTYMinted()
{
...
void RecalculateZBETTYSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zBETTY supply
...
bool RecalculateBETTYSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zBETTY with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zBETTYSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert BETTY to zBETTY
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zBETTY spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zBETTY serial %s in block\n Block: %s",
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if(IsSporkActive(SPORK_20_SPAM_CHK) && (block.GetBlockTime() >= GetSporkValue(SPORK_20_SPAM_CHK))) {
      ...
      if (block.IsProofOfStake()) {
          ...
          // Inputs
          std::vector<CTxIn> bettyInputs;
          std::vector<CTxIn> zBETTYInputs;

          for (CTxIn stakeIn : stakeTxIn.vin) {
              if(stakeIn.scriptSig.IsZerocoinSpend()){
                  zBETTYInputs.push_back(stakeIn);
              }else{
                  bettyInputs.push_back(stakeIn);
              }
          }
          const bool hasBETTYInputs = !bettyInputs.empty();
          const bool hasZBETTYInputs = !zBETTYInputs.empty();
          ...
          for (CTransaction tx : block.vtx) {
              for (CTxIn in: tx.vin) {
                  ...
                  if(hasBETTYInputs)
                      ...
                      for (CTxIn bettyIn : bettyInputs){
                          if(bettyIn.prevout == in.prevout){
          ...
          if (isBlockFromFork) {
              ...
              // Now that this loop if completed. Check if we have zBETTY inputs.
              if(hasZBETTYInputs){
                  for (CTxIn zBettyInput : zBETTYInputs) {
                      CoinSpend spend = TxInToZerocoinSpend(zBettyInput);
                      ...
                      //                                stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zbetty");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zBETTY mints and %d zBETTY spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // BETTY: We use certain sporks during IBD, so check to see if they are
...
{
	return CScript() << ParseHex("047606ed7bb61df565ff37fc4e89172e103cedab1c3380652dde4626690bc914df9e4fc39c15eb0919deef13a070cc1b3be9e5fe75e1b03892e12dc1e564d4b82b") << OP_CHECKSIG;
}

CScript OpFundScriptPubKey()
{
	return CScript() << ParseHex("04c7a26533f681126cb9776e8bb1a27a3984a93294e0ee46f6aa0dd660f6660c95a2eb1a00bdedd8821c3bbaff8cde231ee7a433aa94189556870cfde27b194eba") << OP_CHECKSIG;
}
